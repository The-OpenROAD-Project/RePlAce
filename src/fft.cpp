...
void charge_fft_init_2d(struct POS nbin, struct FPOS stp) {
  ...
  ex_2d_st2 = (float **)malloc(sizeof(float *) * dft_bin_2d.x);
  ey_2d_st2 = (float **)malloc(sizeof(float *) * dft_bin_2d.x);

  for(x = 0; x < dft_bin_2d.x; x++) {
    den_2d_st2[x] = (float *)malloc(sizeof(float) * dft_bin_2d.y);
    phi_2d_st2[x] = (float *)malloc(sizeof(float) * dft_bin_2d.y);
    ex_2d_st2[x] = (float *)malloc(sizeof(float) * dft_bin_2d.y);
  ...
  wx_2d_st = (float *)malloc(sizeof(float) * dft_bin_2d.x);
  wy_2d_st = (float *)malloc(sizeof(float) * dft_bin_2d.y);
  wx2_2d_st = (float *)malloc(sizeof(float) * dft_bin_2d.x);
  wy2_2d_st = (float *)malloc(sizeof(float) * dft_bin_2d.y);
...
void charge_fft_init_3d(struct POS nbin, struct FPOS stp) {
  ...
  DFT_SCALE_3D = 1.0 / ((float)charge_dft_nbin_3d);

  den_3d_st3 = (float ***)malloc(sizeof(float **) * dft_bin_3d.z);
  phi_3d_st3 = (float ***)malloc(sizeof(float **) * dft_bin_3d.z);
  ex_3d_st3 = (float ***)malloc(sizeof(float **) * dft_bin_3d.z);
  ey_3d_st3 = (float ***)malloc(sizeof(float **) * dft_bin_3d.z);
  ez_3d_st3 = (float ***)malloc(sizeof(float **) * dft_bin_3d.z);

  for(z = 0; z < dft_bin_3d.z; z++) {
    den_3d_st3[z] = (float **)malloc(sizeof(float *) * dft_bin_3d.x);
    phi_3d_st3[z] = (float **)malloc(sizeof(float *) * dft_bin_3d.x);
    ex_3d_st3[z] = (float **)malloc(sizeof(float *) * dft_bin_3d.x);
    ey_3d_st3[z] = (float **)malloc(sizeof(float *) * dft_bin_3d.x);
    ez_3d_st3[z] = (float **)malloc(sizeof(float *) * dft_bin_3d.x);

    for(x = 0; x < dft_bin_3d.x; x++) {
      den_3d_st3[z][x] = (float *)malloc(sizeof(float) * dft_bin_3d.y * 2);
      phi_3d_st3[z][x] = (float *)malloc(sizeof(float) * dft_bin_3d.y * 2);
      ex_3d_st3[z][x] = (float *)malloc(sizeof(float) * dft_bin_3d.y * 2);
      ey_3d_st3[z][x] = (float *)malloc(sizeof(float) * dft_bin_3d.y * 2);
      ez_3d_st3[z][x] = (float *)malloc(sizeof(float) * dft_bin_3d.y * 2);
    }
  }

  charge_dft_nbit_3d = 2 + (int)sqrt((float)charge_dft_n_3d + 0.5);
  charge_ip_3d = (int *)malloc(sizeof(int) * charge_dft_nbit_3d);
  charge_dft_nw_3d = (int)(charge_dft_n_3d * 3 / 2) + 1;
  w_3d = (float *)malloc(sizeof(float) * charge_dft_nw_3d);

  charge_ip_3d[0] = 0;

  wx_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.x);
  wy_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.y);
  wz_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.z);

  wx2_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.x);
  wy2_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.y);
  wz2_3d_st = (float *)malloc(sizeof(float) * dft_bin_3d.z);

  for(x = 0; x < dft_bin_3d.x; x++) {
    wx_3d_st[x] = PI * (float)x / ((float)dft_bin_3d.x * stp.x);
    wx2_3d_st[x] = wx_3d_st[x] * wx_3d_st[x];
  }

  for(y = 0; y < dft_bin_3d.y; y++) {
    wy_3d_st[y] = PI * (float)y / ((float)dft_bin_3d.y * stp.y);
    wy2_3d_st[y] = wy_3d_st[y] * wy_3d_st[y];
  }

  for(z = 0; z < dft_bin_3d.z; z++) {
    wz_3d_st[z] = PI * (float)z / ((float)dft_bin_3d.z * stp.z);
...
void charge_fft_call_3d(void) {
  ...
  int n1 = dft_bin_3d.x;
  int n2 = dft_bin_3d.y;
  float a_den = 0, a_phi = 0, denom = 0;
  float a_ex = 0, a_ey = 0, a_ez = 0;
  float wx = 0, wx2 = 0;
  float wy = 0, wy2 = 0;
  float wz = 0, wz2 = 0;
...
//
//  den_2d_st2 = ( float   ** )
//    malloc ( sizeof(float  *) * dft_bin_2d.x ) ;
//  phi_2d_st2 = ( float   ** )
//    malloc ( sizeof(float  *) * dft_bin_2d.x ) ;
//  ex_2d_st2  = ( float   ** )
//    malloc ( sizeof(float  *) * dft_bin_2d.x ) ;
...
//    {
//      den_2d_st2 [x] =
//        ( float   * )malloc ( sizeof(float  ) * dft_bin_2d.y );
//      phi_2d_st2 [x] =
//        ( float   * )malloc ( sizeof(float  ) * dft_bin_2d.y );
//      ex_2d_st2 [x] =
//        ( float   * )malloc ( sizeof(float  ) * dft_bin_2d.y );
...
//  for(x=0;x<dft_bin_2d.x;x++)
//    {
//      wx_2d_st [x] = PI * (float  ) x /
//        ((float  ) dft_bin_2d.x * stp.x);
//      wx2_2d_st [x] = wx_2d_st [x] * wx_2d_st [x] ;
//    }
//
//  for(y=0;y<dft_bin_2d.y;y++)
//    {
//      wy_2d_st [y] = PI * (float  ) y /
//        ((float  ) dft_bin_2d.y * stp.y);
//      wy2_2d_st [y] = wy_2d_st [y] * wy_2d_st [y] ;
//    }
//}
...
//    DFT_SCALE_3D = 1.0 / ((float  )charge_dft_nbin_3d);
//
//    powerDen_3d_st3= (float  ***)malloc(sizeof(float  **)*dft_bin_3d.z);
//    theta_3d_st3   = (float  ***)malloc(sizeof(float  **)*dft_bin_3d.z);
//    heat_x_3d_st3  = (float  ***)malloc(sizeof(float  **)*dft_bin_3d.z);
//    heat_y_3d_st3  = (float  ***)malloc(sizeof(float  **)*dft_bin_3d.z);
//    heat_z_3d_st3  = (float  ***)malloc(sizeof(float  **)*dft_bin_3d.z);
//
//    for (z=0; z<dft_bin_3d.z; z++) {
//        powerDen_3d_st3[z]= (float  **)malloc(sizeof(float  *)*dft_bin_3d.x);
//        theta_3d_st3[z]   = (float  **)malloc(sizeof(float  *)*dft_bin_3d.x);
//        ex_3d_st3[z] = ( float   ** )malloc ( sizeof(float  *) * dft_bin_3d.x );
//        ey_3d_st3[z] = ( float   ** )malloc ( sizeof(float  *) * dft_bin_3d.x );
//        ez_3d_st3[z] = ( float   ** )malloc ( sizeof(float  *) * dft_bin_3d.x );
// 	    for(x=0;x<dft_bin_3d.x;x++)
//	    {
//          den_3d_st3 [z][x] =
//            ( float   *)malloc ( sizeof(float  ) * dft_bin_3d.y * 2 );
//          phi_3d_st3 [z][x] =
//            ( float   *)malloc ( sizeof(float  ) * dft_bin_3d.y * 2 );
//          ex_3d_st3  [z][x] =
//            ( float   *)malloc ( sizeof(float  ) * dft_bin_3d.y * 2 );
...
//  for(x=0;x<dft_bin_3d.x;x++)
//    {
//      wx_3d_st  [x] = PI * (float  ) x /
//        ((float  ) dft_bin_3d.x * stp.x);
//      wx2_3d_st [x] = wx_3d_st [x] * wx_3d_st [x];
//    }
//
//  for(y=0;y<dft_bin_3d.y;y++)
//    {
//      wy_3d_st [y] = PI * (float  ) y /
//        ((float  ) dft_bin_3d.y * stp.y);
//      wy2_3d_st [y] = wy_3d_st [y] * wy_3d_st [y] ;
//    }
//
//  for(z=0;z<dft_bin_3d.z;z++)
//    {
//      wz_3d_st [z] = PI * (float  ) z /
//        ((float  ) dft_bin_3d.z * stp.z);
//      wz2_3d_st [z] = wz_3d_st [z] * wz_3d_st [z] ;
//    }
//
//}

// void copy_phi_from_fft (float   *phi, struct POS p, int flg) {
//    if (flg)    *phi = phi_3d_st3[p.z][p.x][p.y];
//    else        *phi = phi_2d_st2[p.x][p.y];
//}

// void copy_theta_from_fft (float   *theta, struct POS p, int flg) {
...
// void copy_den_to_fft (float   den, struct POS p, int flg) {
//    if (flg)    den_3d_st3[p.z][p.x][p.y] = den;
//    else        den_2d_st2[p.x][p.y] = den;
//}

// void copy_powerDen_to_fft (float   powerDen, struct POS p, int flg) {
