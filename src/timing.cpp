...
void Timing::BuildSteiner( bool scaleApplied ) {
    ...
    for(int i=0; i<_netCnt; i++) {
        ...
        if( curNet->pinCNTinObject == 2 ) {
            ...
            int wl = (!scaleApplied)? fabs( firPin->fp.x - secPin->fp.x ) 
                    + fabs( firPin->fp.y - secPin->fp.y )
                : fabs( firPin->fp.x - secPin->fp.x )* _unitX 
                    + fabs( firPin->fp.y - secPin->fp.y )* _unitY;
//            cout << GetPinName(firPin) << " " <<  GetPinName(secPin) << " " << wl << endl;
            wireSegStor[i].push_back( 
                    wire( PinInfo(firPin), PinInfo(secPin), wl) ); 
            totalStnWL += wl;
//            cout << wl << endl; 
        }
        else {
...
//                cout << GetPinName(curPin) << "("<< x[j] << ", " << y[j] << ") ";
            }
//            cout << endl;
            
            Tree fluteTree = flute(curNet->pinCNTinObject, x, y, FLUTE_ACCURACY, mapping);
...
//                cout << j << "<->" <<  n << endl;
                ...
                PinInfo pin1, pin2;
                auto pinPtr1 = pinMap.find( make_pair(fluteTree.branch[j].x, 
                ...
                auto pinPtr2 = pinMap.find( make_pair(fluteTree.branch[n].x, 
                            fluteTree.branch[n].y));
...
//                    if( pin1.GetData() == 0 && pin1.GetPinNum() == 0) {
