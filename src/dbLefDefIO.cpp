...
void FillReplaceNet(dbSet<dbNet> &nets) {
  ...
  for(nIter = nets.begin(); nIter != nets.end(); ++nIter) {
    ...
    if( curDnet->getBTerms().size() + curDnet->getITerms().size() > 20000 ) {
      cout << "Warning: Huge fan-out detected on " << curDnet->getConstName()  
        << " : " << curDnet->getBTerms().size() + curDnet->getITerms().size() << endl;
      cout << "         Skip for this net to prevent malfunction on FLUTE" << endl;
      continue;
    }
