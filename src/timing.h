...
//    double x_offset, y_offset;           /* COG of VIA relative to the origin of a cell, (in DBU) */
    ...
    cap(0.0), delay(0.0), rTran(0.0), fTran(0.0), driverType(INT_MAX),
//    x_coord(0.0), y_coord(0.0), x_offset(0.0), y_offset(0.0),
    isClock(false), earlySlk(0.0), lateSlk(0.0) {}
};
...
struct net {
    ...
    net(string _name, double _lCap, vector<int> _sources, vector<int> _sinks, int _origIdx) :
        name(_name), lumpedCap(_lCap), sources(_sources), sinks(_sinks), origIdx(_origIdx) {};
};
...
//        }
        ...
        PINNUM_TYPE GetPinNum() const { return pNum; };

        uint32_t GetIdx() const { return data & ( (uint32_t)(1 <<31) - 1 ) ; };
        ...
        inline string GetPinName(void* ptr, vector<vector<string>>& pNameStor) {
            if ( isSteiner() ) {
                cout << "ERROR: GetPinName must be executed only when NOT Steiner Point" << endl;
                exit(1);
            }
...
//                cout << "pname: " << pNameStor[GetIdx()][pNum] << endl;
        ...
        inline string GetStnPinName() {
            if( !isSteiner() ) {
                cout << "ERROR: GetStnPinName must be executed only when Steiner Point" << endl;
                exit(1);
            }
...
//            cout << "data: " << data << " -> " << (data >> 31) << endl;
            return (data >> 31) == 0;};
        bool isSteiner() { return data == numeric_limits<uint32_t>::max(); };
...
//        void ScriptUpdateSta(string& outCutName, int targetCnt);
    ...
    public:
        ...
        Timing( MODULE* modules, TERM* terms, NET* nets, int netCnt,
            ...
            _nets(nets), _netCnt(netCnt),
            _pins(pins), _pinCnt(pinCnt), 
            _mPinName(mPinName), _tPinName(tPinName),
            _unitX(0.0), _unitY(0.0),
            _clkName(clkName), _clkPeriod(clkPeriod),
            scriptIterCnt(0) {
...
inline bool operator==( const PinInfo& lhs, const PinInfo& rhs ) {
    return (lhs.GetData() == rhs.GetData()) && (lhs.GetPinNum()== rhs.GetPinNum()); 
}
